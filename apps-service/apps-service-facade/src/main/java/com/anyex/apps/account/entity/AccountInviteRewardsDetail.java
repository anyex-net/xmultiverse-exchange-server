...
@ApiModel(description = "账户邀请奖励")
public class AccountInviteRewardsDetail extends GenericEntity
{
    @ApiModelProperty(value = "账户ID", position = 2 , required = true )
    @NotNull(message = "账户ID不能为空")
    private Long accountId;

    @ApiModelProperty(value = "'被邀请人账户ID'", position = 3 , required = true )
    @NotNull(message = "'被邀请人账户ID不能为空")
    private Long inviteActId;

    @ApiModelProperty(value = "被邀请人昵称", position = 6 )
    private String inviteActNick;

    @ApiModelProperty(value = "被邀请人头像", position = 6 )
    private String inviteActHead;

    @ApiModelProperty(value = "顺序号", position = 6 , required = true )
    @NotNull(message = "顺序号不能为空")
    private Integer inviteActSeq;
