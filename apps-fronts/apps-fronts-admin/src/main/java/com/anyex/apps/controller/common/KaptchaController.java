...
public class KaptchaController extends GenericController
{
     ...
     */
    @PostMapping(value = "/createKaptcha")
    @ApiOperation(value = "生成随机验证码", httpMethod = "POST")
    public JsonMessage createKaptcha(HttpServletRequest request, @ModelAttribute ReqKaptcha reqKaptcha) throws BusinessException
    {
        ...
        // SMS
        if (MessageConst.ADMIN_VALID_LOGIN.equals(scene))
        {
            RedisUtils.putObject(key.append(MessageConst.ADMIN_VALID_LOGIN).append(GlobalConst.SEPARATOR).append(ip).toString(), captchaText,
...
//                    CacheConst.FIFTEEN_MINUTE_CACHE_TIME);
//        }
