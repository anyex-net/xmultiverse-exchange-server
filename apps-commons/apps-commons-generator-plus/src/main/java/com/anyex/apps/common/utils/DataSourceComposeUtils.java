...
public class DataSourceComposeUtils {
     ...
     * 达梦连接字符串模板
     */
    private static String URL_TEMPLATE_DM = "jdbc:dm://{}:{}?schema={}&compatibleMode=mysql&characterEncoding=UTF-8&useUnicode=true&useSSL=false&tinyInt1isBit=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Shanghai&rewriteBatchedStatements=true";
    ...
    public static DruidDataSource composeDruidDataSource(SysDataSource dataSource) throws BusinessException {
        ...
        }  else if (StringUtils.equals(dataSource.getDbType(), Constants.DATABASE_TYPE_DM)) {
            druidDataSource.setUrl(StringUtils.format(URL_TEMPLATE_DM, dataSource.getHost(),
                    dataSource.getPort(), dataSource.getName()));
        } else {
