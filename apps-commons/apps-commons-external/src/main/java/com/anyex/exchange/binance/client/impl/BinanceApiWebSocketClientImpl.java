...
import com.anyex.exchange.binance.client.BinanceApiWebSocketClient;
import com.anyex.exchange.binance.client.config.BinanceApiConfig;
import com.anyex.exchange.binance.client.domain.event.*;
...
public class BinanceApiWebSocketClientImpl implements BinanceApiWebSocketClient, Closeable {
    ...
    public Closeable onDApiUserDataUpdateEvent(String listenKey, BinanceApiCallback<UserDataUpdateEvent> callback) {
        return createDapiNewWebSocket(listenKey, new BinanceApiWebSocketListener<>(callback, UserDataUpdateEvent.class));
    }

    public Closeable onFApiUserDataUpdateEvent(String listenKey, BinanceApiCallback<UserDataUpdateEvent> callback) {
        return createFapiNewWebSocket(listenKey, new BinanceApiWebSocketListener<>(callback, UserDataUpdateEvent.class));
    }
    ...
    public Closeable onAllCoinContractMarketTickersEvent(BinanceApiCallback<List<TickerEvent>> callback) {
        final String channel = "!miniTicker@arr";
        return createDapiNewWebSocket(channel, new BinanceApiWebSocketListener<>(callback, new TypeReference<List<TickerEvent>>() {
        }));
    }

    public Closeable onAllUsdtContractMarketTickersEvent(BinanceApiCallback<List<TickerEvent>> callback) {
        final String channel = "!miniTicker@arr";
        return createFapiNewWebSocket(channel, new BinanceApiWebSocketListener<>(callback, new TypeReference<List<TickerEvent>>() {
        }));
    }
    ...
    private Closeable createDapiNewWebSocket(String channel, BinanceApiWebSocketListener<?> listener) {
        String streamingUrl = String.format("%s/%s", BinanceApiConfig.useTestnetStreaming?BinanceApiConfig.getStreamTestNetBaseUrl():BinanceApiConfig.getDApiStreamApiBaseUrl(), channel);
        Request request = new Request.Builder().url(streamingUrl).build();
        final WebSocket webSocket = client.newWebSocket(request, listener);
        return () -> {
            final int code = 1000;
    ...
    private Closeable createFapiNewWebSocket(String channel, BinanceApiWebSocketListener<?> listener) {
        String streamingUrl = String.format("%s/%s", BinanceApiConfig.useTestnetStreaming?BinanceApiConfig.getStreamTestNetBaseUrl():BinanceApiConfig.getFApiStreamApiBaseUrl(), channel);
        Request request = new Request.Builder().url(streamingUrl).build();
        final WebSocket webSocket = client.newWebSocket(request, listener);
        return () -> {
            final int code = 1000;
