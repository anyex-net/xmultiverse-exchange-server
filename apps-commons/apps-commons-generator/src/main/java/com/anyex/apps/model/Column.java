...
import com.anyex.apps.utils.DatabaseDataTypesUtils;
import com.anyex.apps.utils.StringHelper;
...
public class Column
{
    ...
    public Column(Table table, int sqlType, String sqlTypeName, String sqlName, int size, int decimalDigits, boolean isPk, boolean isFk, boolean isNullable,
            boolean isIndexed, boolean isUnique, String defaultValue, String sqlRemark,int sort)
    {
        _table = table;
        _sqlType = sqlType;
        _sqlName = sqlName;
    ...
    public String getSqlTypeName()
    {
        ...
        if ("INT".equals(_sqlTypeName)) _sqlTypeName = "INTEGER";
        if ("TEXT".equals(_sqlTypeName)) _sqlTypeName = "CLOB";
        return _sqlTypeName;
    }
    ...
    {
        if (_sqlRemark != null && _sqlRemark.length() > 0)
        { return _sqlRemark; }
