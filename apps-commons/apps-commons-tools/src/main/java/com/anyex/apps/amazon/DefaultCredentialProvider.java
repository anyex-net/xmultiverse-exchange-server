...
public class DefaultCredentialProvider implements CredentialsProvider
{
    ...
    {
        if (creds == null)
        { throw new BusinessException("creds should not be null."); }
    ...
    private static void checkCredentials(String accessKeyId, String secretAccessKey)
    {
        ...
        { throw new BusinessException("Access key id should not be null or empty."); }
        if (secretAccessKey == null || secretAccessKey.equals(""))
        { throw new BusinessException("Secret access key should not be null or empty."); }
    }
}