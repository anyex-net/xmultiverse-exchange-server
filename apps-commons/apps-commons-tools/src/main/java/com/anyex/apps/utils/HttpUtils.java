...
public class HttpUtils
{
    ...
    {
        return get(getHttpClient(), uri, map, null);
    }
    ...
    private static String executeRequest(HttpClient httpClient, HttpRequestBase request, String charsetName, Map<String, String> header)
    {
        ...
        try
        {
            // log.info("httputils execute url:{}",request.getURI());
            // log.info("httputils execute request:{}",JSONUtils.beanToJson(request));
            HttpResponse response = httpClient.execute(request);
            if(null == response)
            {
                log.error("{}接口返回异常:response 为空",request.getURI());
                // throw new BusinessException(request.getURI()+"接口返回异常:response 为空");
            }
             // log.info("httputils execute response:{}",JSONUtils.beanToJson(response));
            // 获取状态
    ...
    {
        return postWithJSON(getHttpClient(), null, url, json);
    }
